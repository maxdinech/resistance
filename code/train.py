...
Note: the -split s argument is used to divide the training images into `train`
and `val`, with proportions respectively `s` and `(1-s)`.
...
from basics import load_architecture
import data_loader
import plot
...
try:
    ...
    for e in range(epochs):
        ...
        print(f" |-> train_acc@{k}: {train_acc:5.2f}%",
              f" -- train_loss: {train_loss:6.4f}")
        print(f" |->   val_acc@{k}: {val_acc:5.2f}%",
