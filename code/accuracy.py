...
Note: to compute the accuracy on `train` or `val`, the -split argument is
needed in order to determine the right repartition of all training images into
`train` and `val`.  To compute the accuracy over all training images at once,
use:
        python3 accuracy.py [-k K] model dataset train -split 0
...
def accuracy(images, labels, k=1):
    ...
    for (x, y) in bar(loader):
        ...
        count += sum(sum((y_pred_k.t() == y).double()))
        # .double(): ByteTensor sums are limited at 255.
